...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('login')  # show error and stay on login page
...
def todo_view(request):
    if request.method == 'POST':
        ...
        Todo.objects.create(user=request.user, task=task, priority=priority, status=status)
...
def activation_email(request, username):
    try:
        ...
        return redirect('login')  # or the name of your login URL
        ...
        if User.objects.filter(username=username).exists():
            ...
            if not User.objects.filter(username=username).exists():
                user = User.objects.create_user(
                    ...
                    first_name=first_name,
                    last_name=last_name,
                    is_active=True  # Let login happen only after verification flag
                )
